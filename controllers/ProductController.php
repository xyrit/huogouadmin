...
class ProductController extends BaseController
{
    public function actionIndex()
    {
        ...
        if ($request->isAjax) {
            ...
            $condition['limit_num'] = $request->get('limit_num', 'all');
            $condition['buy_unit'] = $request->get('buy_unit', 'all');
            $condition['sort'] = $request->get('sort', '');
            $condition['order'] = $request->get('order', '');
            $data = Product::getList($condition, $page, $pageSize);
    ...
    public function actionAdd()
    {
        ...
        if ($request->isPost) {
            $post = $request->post();
            $model->display = $post['Product']['display'];
            $post['Product']['delivery_id'] = isset($post['Product']['delivery_id']) ? implode(',', $post['Product']['delivery_id']) : '';
            if ($model->load($post)) {
            ...
            try {
                ...
                if (($post['cat_id'] != $model['cat_id'] || $post['brand_id'] != $model['brand_id'])) {
                    ...
                    if ($productModel) {
                        ...
                        Period::updateAll(['cat_id'=>$post['cat_id']],['product_id'=>$id]);
                        ShareTopic::updateAll(['cat_id'=>$post['cat_id']],['product_id'=>$id]);
    ...
    public function actionMarket()
    {
        ...
        if ($request->isAjax) {
            ...
            if (!Product::market($product, $market)) {
    ...
    {
        $product = Product::find()->distinct('name')->asArray()->all();
        $result = [];
    ...
    public function actionSearch()
    {
        ...
        $list = Product::find()->where($where)->asArray()->all();
    ...
    public function actionGetInfo()
    {
        ...
        return Product::find()->where(['id'=>$id])->asArray()->one();
    }
}