...
*/
var fnImageCropRot = function(o, iOrigWidth, iOrigHeight){
	//o为图片对象
	if(typeof(o) === "object" && o.nodeName.toLowerCase() === "img"){//检测是否是图片类型的DOM对象
		...
		if(iCurWidth && iOrigWidth){//如果宽度为不为0 - 意味着加载成功
			...
			/*ID("zxxRotAngle").onkeyup = function(){
				...
				if (v%90 == 0) {
					//iCurWidth = oCanvas.width;
					//iCurHeight = oCanvas.height;
				}
			};*/

			ID("zxxRotAngle").onclick = function() {
				fnCanvasRotate(oCanvas, o, angle);
				$('#zxxRotAngle').attr('angle', angle);
				angle += 90;
				if (angle >= 360) {
					angle = 0;
				}
				...
				document.onmousemove = function(event){
					...
					if(params.flag){
						...
						if(params.kind === "n"){
							...
							if (width / height < oCanvas.width / oCanvas.height && height >= oCanvas.height) {
								target.style.height = oCanvas.height + "px";
								target.style.width = oCanvas.height * percent + "px";
							} else if (width / height > oCanvas.width / oCanvas.height && width >= oCanvas.width) {
								target.style.width = oCanvas.width + "px";
								target.style.height = oCanvas.width / percent + "px";
							} else {
							...
							if (left + parseInt(target.style.width) > oCanvas.width) {
								target.style.left = (oCanvas.width - parseInt(target.style.width)) + "px";
							}
							if (parseInt(target.style.top) + parseInt(target.style.height) > oCanvas.height) {
								target.style.top = (oCanvas.height - parseInt(target.style.height)) + "px";
							}
						}
					}
				}	
